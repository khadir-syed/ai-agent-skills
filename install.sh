...
usage() {
  ...
  echo "  <skill-name> must be a skill folder under skills/<domain>/, agents/<domain>/, or orchestrators/" >&2
  exit 1
}
...
matches=()
for f in "$script_dir"/skills/*/"$skill" "$script_dir"/agents/*/"$skill" "$script_dir"/orchestrators/"$skill"; do
  [ -f "$f/SKILL.md" ] && matches+=("$f")
done

case "${#matches[@]}" in
  0) echo "No such skill: $skill (looked under skills/*/$skill, agents/*/$skill, and orchestrators/$skill)" >&2; exit 1 ;;
  1) src="${matches[0]}" ;;
