---
name: feature-launch-readiness-agent-controlled
description: Chain the Requirements Gap Investigator, a launch brief draft, and the Feature Launch Readiness Checklist, pausing for approval after every step. Use for the same job as the autonomous variant when you want a checkpoint at every hand-off, not just before the write.
---

# Feature Launch Readiness Agent — Controlled

Run the same three steps as
[../feature-launch-readiness-agent-autonomous](../feature-launch-readiness-agent-autonomous/)
...
- Carry forward each underlying skill's own operating boundary for its step:
  gap investigation does not draft; the checklist does not decide or edit.
- The only file write in this chain is the launch brief in step 2, and it
...
Run the same investigation as the Requirements Gap Investigator: frame the
...
Once approved to proceed, draft the launch brief from the gap report — see
[references/launch-readiness-format.md](references/launch-readiness-format.md).
...
Once the draft is written, run the same six checks as the Feature Launch
...
- the user asks this agent to also decide whether to launch; or
